...
void attack(Matrix& opponent)
{
    char x;
    int y;
    cout << "Write the coordinates you want to attack, write first Letter then Number" << endl;
    cin >> x; 
    cin >> y;
    int convertedX = (int) (x-65);
    int convertedY = (y-1);
    isvalid(convertedX, convertedY);
    if (opponent.grid[convertedX][convertedY]==Matrix::Water)
    {
        opponent.grid[convertedX][convertedY] = Matrix::Miss;
    }
    else if(opponent.grid[convertedX][convertedY]== Matrix::Ship)
    {
        opponent.grid[convertedX][convertedY] = Matrix::Hit;
        cout << "\a";                                                                     // Ljud tillägg
    }
    

}
// SpecialAttack som skjuter på random koordinater
void rapidFireCanon(Matrix& opponent)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        int x = rand() % Matrix::xRow;
        int y = rand() % Matrix::yRow;
...
int main()
{
    ...
    displayOceanPlayer(ocean[0], false);
